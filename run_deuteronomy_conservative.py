...
# -*- coding: utf-8 -*-
"""
Process complete book of Deuteronomy with conservative API
"""
import sys
import os
...
def process_deuteronomy_conservative():
    """Process complete Deuteronomy with conservative API"""

    print("=== PROCESSING COMPLETE DEUTERONOMY WITH CONSERVATIVE API ===")
    ...
    for chapter in range(1, 35):
        print(f"\n--- Processing Deuteronomy {chapter} ---")

        # Get text
        verses_data, api_time = sefaria.extract_hebrew_text(f"Deuteronomy.{chapter}")

        if not verses_data:
            print(f"Failed to get text for Deuteronomy {chapter}")
            errors += 1
            continue
        ...
        print(f"Deuteronomy {chapter}: {chapter_instances} figurative instances")
...
if __name__ == "__main__":
    process_deuteronomy_conservative()