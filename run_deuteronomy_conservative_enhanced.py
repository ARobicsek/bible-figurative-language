...
# -*- coding: utf-8 -*-
"""
Enhanced Deuteronomy processing with comprehensive error logging and resumption capability
"""
import sys
import os
import logging
...
def check_existing_progress(db_name):
    """Check what chapters have already been processed"""
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT DISTINCT chapter FROM verses WHERE book = 'Deuteronomy' ORDER BY chapter")
...
def process_deuteronomy_conservative_enhanced():
    """Process complete Deuteronomy with enhanced error handling and resumption"""

    # Setup logging
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_file = f"deuteronomy_conservative_log_{timestamp}.txt"
    logger = setup_logging(log_file)

    logger.info("=== PROCESSING COMPLETE DEUTERONOMY WITH CONSERVATIVE API (ENHANCED) ===")
    ...
    try:
        ...
        # Check for existing database and resume from where left off
        db_name = f"deuteronomy_conservative_{timestamp}.db"
        existing_db = "deuteronomy_conservative_20250920_200506.db"

        if os.path.exists(existing_db):
            logger.info(f"Found existing database: {existing_db}")
            completed_chapters = check_existing_progress(existing_db)
            logger.info(f"Already completed chapters: {sorted(completed_chapters)}")
            ...
            try:
                ...
                cursor.execute("SELECT COUNT(*) FROM verses LIMIT 1")
        ...
        for chapter in range(1, 35):
            if chapter in completed_chapters:
                logger.info(f"--- Skipping Deuteronomy {chapter} (already completed) ---")
                continue

            chapter_start_time = time.time()
            logger.info(f"--- Processing Deuteronomy {chapter} ---")

            try:
                # Get text with error handling
                logger.info(f"Fetching text for Deuteronomy {chapter}...")
                verses_data, api_time = sefaria.extract_hebrew_text(f"Deuteronomy.{chapter}")

                if not verses_data:
                    error_msg = f"Failed to get text for Deuteronomy {chapter} - verses_data is None/empty"
                    logger.error(error_msg)
                    errors += 1
                    chapter_progress[chapter] = {"status": "failed", "error": "No text data", "verses": 0, "instances": 0}
                    continue

                logger.info(f"Retrieved {len(verses_data)} verses for Deuteronomy {chapter}")
                chapter_instances = 0
                chapter_verses = 0
                ...
                logger.info(f"Deuteronomy {chapter} COMPLETED: {chapter_instances} figurative instances from {chapter_verses} verses ({chapter_time/60:.1f} min)")

                # Commit after each chapter
                db_manager.conn.commit()

            except Exception as e:
                error_msg = f"Critical error processing Deuteronomy {chapter}: {str(e)}"
                logger.error(error_msg)
                logger.error(f"Traceback: {traceback.format_exc()}")
        ...
        completed_chapters_new = sum(1 for ch in chapter_progress.values() if ch["status"] == "completed")
        failed_chapters = sum(1 for ch in chapter_progress.values() if ch["status"] == "failed")

        logger.info(f"New chapters completed: {completed_chapters_new}/34")
        logger.info(f"Chapters failed: {failed_chapters}/34")
        ...
        summary_file = f"deuteronomy_processing_summary_{timestamp}.json"
        summary_data = {
            "database": db_name,
            "log_file": log_file,
            ...
            "errors": errors,
            "chapters_completed": completed_chapters_new,
...
if __name__ == "__main__":
    process_deuteronomy_conservative_enhanced()