...
# -*- coding: utf-8 -*-
"""
Process complete Genesis with multi-model API (Gemini 2.5 Flash → 1.5 Flash fallback)
...
def main():
    ...
    db_name = f"genesis_multi_model_{timestamp}.db"
    log_name = f"genesis_multi_model_log_{timestamp}.txt"

    setup_logging(log_name)

    logging.info("=== PROCESSING GENESIS WITH MULTI-MODEL API ===")
    ...
    # Process all Genesis chapters (1-50)
    ...
    for chapter in range(1, 51):  # Genesis has 50 chapters
        try:
            result = process_chapter(sefaria_client, gemini_client, "Genesis", chapter, db_name)
            ...
            logging.error(f"Exception processing Genesis {chapter}: {e}")

    # Final summary
    total_end = datetime.now()
    total_time = (total_end - total_start).total_seconds()

    logging.info("\n=== GENESIS MULTI-MODEL PROCESSING COMPLETE ===")
    logging.info(f"Database: {db_name}")
    logging.info(f"Log file: {log_name}")
    ...
    # Save summary
    summary_file = f"genesis_multi_model_summary_{timestamp}.json"
